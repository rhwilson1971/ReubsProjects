...
(
	REQUESTOR_ID INTEGER NOT NULL PRIMARY KEY GENERATED ALWAYS AS IDENTITY (START WITH 1, INCREMENT BY 1),
...
(
	REQUEST_ID INTEGER NOT NULL PRIMARY KEY GENERATED ALWAYS AS IDENTITY (START WITH 1, INCREMENT BY 1),
...
ALTER TABLE PRAYER_REQUEST ADD CONSTRAINT REQUESTOR_FK
Foreign Key (REQUESTOR_ID) REFERENCES PRAYER_REQUESTOR (REQUESTOR_ID);

create table PRAYER_RESPONSE
(
	RESPONSE_ID INTEGER NOT NULL PRIMARY KEY GENERATED ALWAYS AS IDENTITY (START WITH 1, INCREMENT BY 1),
...
ALTER TABLE PRAYER_RESPONSE ADD CONSTRAINT REQUEST_FK
Foreign Key (REQUEST_ID) REFERENCES PRAYER_REQUEST (REQUEST_ID);

create table LKUP_DISPOSITION
(
	DISPOSITION_ID INTEGER NOT NULL PRIMARY KEY GENERATED ALWAYS AS IDENTITY (START WITH 1, INCREMENT BY 1),
	DISPOSITION VARCHAR(100) not null
);
...
insert into prayer_requestor (NAME, EMAIL, PASSWORD, LAST_LOGON_DATE, LAST_LOGON_TIME, ISACTIVE, ISADMIN)
	values ('anonymous', 'anonymous@wilmss.com', 'anonymous', NULL, NULL, 1, 0);
